...
import {AddCategorie, AddCategorieProps} from "../categorie/AddCategorie";
...
export const AddStandort = (props: AddStandortProps) => {
    ...
    const addStandort = async () => {
        try {
            const response = await firestore().collection(abteilungenCollection).doc(abteilungId).collection(abteilungenStandortCollection).add(form.getFieldsValue() as Standort)
            if (response.id) {
                message.success(`Standort ${form.getFieldValue('name')} erfolgreich erstellt`);
                form.resetFields();
                if (onSuccess) {
                    onSuccess()
                }
...
export const AddStandortButton = (props: AddStandortProps) => {
    ...
    return <>
        ...
        >
            <AddStandort abteilungId={abteilungId} onSuccess={() => {
                setIsModalVisible(false)
            }}/>
        </Modal>
    </>
